# Create Nova Sonic WebSocket Infrastructure
# Creates all required AWS resources step by step

Write-Host "🏗️ Creating Nova Sonic WebSocket Infrastructure..." -ForegroundColor Green
...
Write-Host "Attaching basic Lambda execution policy..." -ForegroundColor Cyan
...
Set-Location "lambda\nova-websocket-handler"

if (Test-Path "nova-websocket-handler.zip") {
    Remove-Item "nova-websocket-handler.zip"
}

Write-Host "Creating deployment package..." -ForegroundColor Cyan
Compress-Archive -Path "index.js", "package.json", "node_modules" -DestinationPath "nova-websocket-handler.zip" -Force
...
if ($LASTEXITCODE -ne 0) {
    ...
    aws lambda create-function `
        ...
        --zip-file fileb://lambda/nova-websocket-handler/nova-websocket-handler.zip `
        --timeout 300 `
        --memory-size 512 `
        --environment "Variables={CONNECTIONS_TABLE=$CONNECTIONS_TABLE,SESSIONS_TABLE=$SESSIONS_TABLE,COGNITO_IDENTITY_POOL_ID=us-east-1:d030a5b5-e950-493c-855f-a578cc578e39,COGNITO_USER_POOL_ID=us-east-1_BxbAO9DtG,NOVA_SONIC_MODEL_ID=amazon.nova-sonic-v1:0}"
    ...
    Write-Host "✅ Lambda function already exists (updating code)" -ForegroundColor Green
...
$integrationExists = aws apigatewayv2 get-integrations --api-id $API_ID --query "Items[0].IntegrationId" --output text 2>$null
...
Write-Host ""
Write-Host "🛤️ Step 6: Creating WebSocket Routes..." -ForegroundColor Yellow
...
foreach ($route in $routes) {
    ...
    if ([string]::IsNullOrEmpty($routeExists) -or $routeExists -eq "None") {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "⚠️ Warning: Failed to create route $route" -ForegroundColor Yellow
        }
    } else {
...
if ([string]::IsNullOrEmpty($stageExists) -or $stageExists -eq "None") {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "❌ Failed to create production stage" -ForegroundColor Red
        exit 1
    }
} else {
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "⚠️ Warning: Deployment may have failed" -ForegroundColor Yellow
}
