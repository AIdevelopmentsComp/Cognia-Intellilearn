...
function Remove-EmptyLogGroups {
    ...
    try {
        ...
        foreach ($logGroup in $emptyGroups) {
            ...
            if ($name -match "cognia-bedrock-voice-streaming") {
                continue
            }
...
Write-Host "  - S3: intellilearn-final (website hosting)" -ForegroundColor White
Write-Host "  - S3: cognia-intellilearn (content storage)" -ForegroundColor White
Write-Host "  - Lambda: cognia-bedrock-voice-streaming" -ForegroundColor White
